...
interface InfluencerApplicationActionsProps {
  applicationId: string
  currentStatus: string
}

export default function InfluencerApplicationActions({ 
  applicationId, 
  currentStatus 
}: InfluencerApplicationActionsProps) {
  ...
  const handleAction = async (action: 'approve' | 'reject') => {
    ...
    const confirmMessage = action === 'approve' 
      ? 'Are you sure you want to approve this influencer application?' 
      : 'Are you sure you want to reject this influencer application?'
    
    if (!confirm(confirmMessage)) return
    
    setIsLoading(true)
    try {
      const response = await fetch(`/api/admin/applications/influencer/${applicationId}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleStatusChange = async (newStatus: 'pending' | 'approved' | 'rejected') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json()
        alert(`Error: ${errorData.error || 'Failed to change status'}`)
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Status Management for approved/rejected */}
      {currentStatus !== 'pending' && (
        <div className="space-y-3">
          ...
          {showStatusChange && (
            <div className="space-y-2 p-3 bg-gray-50 rounded-lg">
              ...
              <button
                ...
                className="w-full px-3 py-2 text-sm bg-yellow-100 text-yellow-800 rounded-md hover:bg-yellow-200 transition-colors disabled:opacity-50"
              >
                Pending Review
              </button>
              <button
                ...
                className="w-full px-3 py-2 text-sm bg-green-100 text-green-800 rounded-md hover:bg-green-200 transition-colors disabled:opacity-50"
              >
                Approved
              </button>
              <button
                ...
                className="w-full px-3 py-2 text-sm bg-red-100 text-red-800 rounded-md hover:bg-red-200 transition-colors disabled:opacity-50"
              >
                Rejected
              </button>
              <button
      ...
      <div className="border-t pt-4 space-y-2">
        ...
        <button
          onClick={() => {
            const mailto = `mailto:${applicationId}@temp.com?subject=Regarding your influencer application`
            window.open(mailto)
          }}
          className="w-full px-3 py-2 text-sm bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors"
        >
